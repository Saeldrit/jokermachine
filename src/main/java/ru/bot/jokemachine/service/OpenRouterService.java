...
public class OpenRouterService {
	...
	private static class Promt {
		...
		private static final String FRY_MYSELF = """
				...
				Напиши небольшую стендап прожарку (максимум 4 предложения) используя сарказм и юмор.
				Используй его имя '%s' и пол.
				Подчеркни слабости и нелепые ситуации.
				Используй мат в прожарке, если он используется в сообщениях пользователя.
		...
		private static final String DESTINY_MYSELF = """
				...
				На основе имени пользователя '%s' и манере общения,
				предскажи его судьбу на сегодняшний день и ближайшее будущее.
				Не используй знак зодиака.
				Построй ответ в загадочной манере пророчества свойственной предсказаниям,
				как в Ведьмаке, Властелин колец. Напиши предсказание максимум в 4 предложениях.
				Набор сообщений пользователя:
				'%s'
				""";
	}
